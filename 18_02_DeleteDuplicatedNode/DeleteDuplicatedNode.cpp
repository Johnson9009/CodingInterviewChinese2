...
// 面试题18(二): 删除链表中重复的结点
// 题目: 在一个排序的链表中, 如何删除重复的结点? 例如, 在图3.4(a)中重复结点被删除之后, 链表如图3.4
// (b)所示.
...
void DeleteDuplication(ListNode** head) {
  ...
  if ((*head == nullptr) || ((*head)->m_pNext == nullptr)) return;
  ListNode** ahead = head;
  do {
    while ((*ahead)->m_pNext->m_nValue != (*ahead)->m_nValue) {
      ahead = &((*ahead)->m_pNext);
      if ((*ahead)->m_pNext == nullptr) return;
    }
    ListNode* delete_node = *ahead;
    int value = delete_node->m_nValue;
    while ((delete_node != nullptr) && (delete_node->m_nValue == value)) {
      ListNode* tmp = delete_node;
      delete_node = delete_node->m_pNext;
      delete tmp;
    }
    *ahead = delete_node;
  } while (((*ahead) != nullptr) && ((*ahead)->m_pNext != nullptr));
  return;
}

// ====================测试代码====================
void Test(char const* testName, ListNode** pHead, int* expectedValues, int expectedLength) {
  ...
  while (pNode != nullptr && index < expectedLength) {
    if (pNode->m_nValue != expectedValues[index]) break;
